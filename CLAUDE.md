...
- 사용자가 소유·관리하는 공유기는, 사용자가 직접 제공한 관리자 자격 증명으로 로그인해 DHCP 접속 목록 등 관리 정보를 **읽기 전용**으로 조회할 수 있다. 이는 정당한 인증 접근이며 인증 우회가 아니다.
- 취약점 악용, 비밀번호 대입(추측), 무단 패치, 공유기 설정 변경은 범위 밖이다. 공유기 연동은 사용자 제공 자격 증명만 사용하고, 읽기 전용을 유지하며, 조회 데이터를 외부로 전송하지 않는다.
