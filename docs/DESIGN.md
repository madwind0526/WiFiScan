...
아이콘 중심 표현을 유지하되 Tooltip과 Semantics label을 함께 제공한다. 설정에는 시스템/라이트/다크 테마를 두며 향후 표시 옵션과 탐색 옵션을 확장할 수 있게 한다.
...
현재 연결된 Wi-Fi 하나만으로는 별도 공유기·서브넷의 장비를 볼 수 없다. 사용자가 등록한 네트워크 프로필을 순서대로 연결하고 각 네트워크를 독립적으로 검색한 뒤 결과를 통합한다. Windows는 OS에 저장된 WLAN 프로필을 사용하고, 새 암호는 연결 시 임시 프로필로만 전달하며 앱 파일에는 저장하지 않는다. Android는 `WifiNetworkSpecifier` 시스템 승인 흐름을 사용하며 기기별 동시 연결 지원 여부에 따라 결과가 달라질 수 있다.
