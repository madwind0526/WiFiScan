...
기본 ICMP/neighbor 탐색 결과를 다음 독립 제공자에 전달하고, 제공자 하나가 실패해도 나머지 결과를 합칠 수 있도록 구성한다.

- 역방향 DNS: 운영체제가 확인할 수 있는 호스트 이름을 수집한다.
- mDNS/DNS-SD: 장비가 공개한 서비스 인스턴스, 호스트 이름, 모델·제품 TXT 값을 수집한다.
- SSDP/UPnP: 발견된 장비 IP가 직접 제공하는 설명 문서만 읽어 이름, 제조사, 모델, 설명을 수집한다. 다른 호스트로 향하는 LOCATION과 HTTP 리디렉션은 요청하지 않는다.
- TCP 연결 확인: 지정된 15개 일반 서비스 포트에 연결 가능 여부만 확인하며 애플리케이션 데이터는 전송하지 않는다.

보강 결과에는 모델, 설명, 호스트 이름, 서비스 프로토콜·포트·전송 계층·관측 출처가 저장된다. Android에서는 mDNS 수집 동안에만 멀티캐스트 잠금을 획득하고 반드시 해제한다. Windows에서는 mDNS 소켓을 `reusePort: false`로 생성해 플랫폼 소켓 제약을 피한다.

서비스 포트가 열려 있다는 관측은 취약점 확정이 아니다. FTP/Telnet, HTTP 단독 노출, MQTT는 확인이 필요한 발견사항으로만 표시한다. 특정 CVE 또는 펌웨어 취약점은 정확한 제품과 현재 버전, 공식 근거를 함께 확인할 수 있을 때만 연결한다.

사용자 공유기 `ipTIME A6004NS-M`은 모델 관측 시 공식 펌웨어 확인 안내를 표시한다. 현재 확인한 ipTIME 공개 자료에는 이 모델의 외부 관리 API가 문서화되어 있지 않으므로, 인증 우회나 관리 페이지 역공학 대신 비인증 탐색과 수동 펌웨어 확인까지만 지원한다. 향후 읽기 전용 커넥터는 공식 API 또는 사용자가 승인한 검증된 로컬 인터페이스가 있을 때 추가한다.

### Wave 11 Windows Wi-Fi 인터페이스 경계

Windows에서 Ethernet과 Wi-Fi가 동시에 기본 게이트웨이를 가져도 현재 SSID 스캔은 `NdisPhysicalMedium == 9`인 활성 Wi-Fi 인터페이스만 사용한다. Wi-Fi가 없으면 Ethernet으로 대체하지 않고 Wi-Fi 연결 안내를 표시한다. 따라서 `192.168.45.x` Wi-Fi에 연결한 검색은 Ethernet의 `192.168.0.1`이 아니라 Wi-Fi가 받은 게이트웨이와 이웃 테이블을 사용한다.

2.4G/5G SSID가 같은 게이트웨이와 서브넷을 공유하면 L3 ICMP/neighbor 검색으로는 AP별 실제 연결 단말을 구분할 수 없다. 이 경우 UI는 결과를 “연결 장비” 대신 “서브넷 관측”으로 표시하고 게이트웨이를 함께 보여준다. SSID·밴드별 실제 접속 단말 구분은 사용자 인증을 거친 공유기 접속자 목록 또는 공식 관리 API가 필요하다.
...
- [Multicast DNS RFC 6762](https://www.rfc-editor.org/info/rfc6762/)
- [DNS-Based Service Discovery RFC 6763](https://www.rfc-editor.org/info/rfc6763/)
- [ipTIME A6004NS-M 제품 정보](https://iptime.com/iptime/?page=&page_id=11&pd=7&pf=3&pt=332)
