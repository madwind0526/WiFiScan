...
사용자 공유기 `ipTIME A6004NS-M`은 모델 관측 시 공식 펌웨어 확인 안내를 표시한다. 사용자가 소유한 공유기이므로, 사용자가 직접 제공한 관리자 자격 증명으로 로그인해 DHCP 접속 목록·호스트네임을 **읽기 전용**으로 조회하는 커넥터를 지원한다(정당한 인증 접근). 이 커넥터는 비밀번호를 추측하지 않고 사용자가 준 값만 사용하며, 공유기 설정을 변경하지 않고, 조회 결과를 외부로 전송하지 않는다. 자격 증명은 OS 보안 저장소 또는 로컬 환경 변수에 보관한다.
...
### Wave 14 Obsidian 스타일 메시 그래프

기존의 고정 방사형 배치를 결정론적 힘 기반 배치로 교체한다. 같은 노드·간선 입력은 항상 같은 위치를 만들어 재스캔이나 호버 시 그래프가 불필요하게 흔들리지 않는다. 간선은 스캔 레코드에서 확인한 공유기-장비 관계만 표시하며, 서로 다른 게이트웨이를 임의로 연결하지 않는다.

노드는 아이콘 대신 입체 구로 표시한다. Wi-Fi 공유기는 가장 크게, PC·핸드폰·모니터/TV·가전제품·기타는 유형별 색상으로 표시하고 오른쪽 세로 범례을 제공한다. 호버한 노드와 이웃 관계는 강조하고 나머지는 흐리게 표시한다. 노드 탭은 기존 장비 상세 창을 열며, 드래그 이동·스크롤 줌·확대·축소·전체 맞춤을 지원한다. 릴리스 버전은 `1.2.0+5`이다.

### Wave 15 단색 메시 원과 공개 식별 정보 보강

메시 노드는 입체 그라데이션 대신 유형별 단색 원을 사용한다. 간선 좌표와 원 위젯의 실제 중심을 같은 좌표로 맞추고 신규 장비 빨간 점과 반복되는 `확인되지 않은 장비` 라벨을 제거한다. 현재 장비와 게이트웨이는 `내 PC`, `GW`로 줄이고, 다른 노드 라벨은 5자를 넘으면 `...`으로 줄인다. 오른쪽 아래 범례는 제목 없이 `WiFi`, `PC`, `Phone`, `TV`, `Home` 라벨을 원 왼쪽에 오른쪽 정렬한다.

여러 네트워크의 GW와 장비는 별도 섬이나 표시된 영역 원으로 나누지 않고 하나의 원형 힘 기반 클라우드에 함께 배치한다. 각 장비는 실제 관측된 GW 간선만 표시하며, GW 사이에는 화면에 그리지 않는 배치용 인력만 적용해 여러 별 모양 성단이 Obsidian 그래프처럼 자연스럽게 섞이도록 한다. 스캔 결과 갱신 직후에는 전체 맞춤을 다음 프레임에도 확인해 확대 상태가 남지 않게 한다. 상단 오류 메시지는 반투명 오류색과 오른쪽 닫기 버튼을 사용한다.

장비 식별은 외부 MAC 조회 API로 로컬 식별자를 보내지 않는다. RFC 6763의 `_services._dns-sd._udp.local` 유형 열거로 하드코딩 목록 밖의 공개 서비스까지 찾고, Windows에서는 제한된 동시 실행과 시간 제한으로 `nbtstat /A` 원격 이름표를 읽는다. 이름, 모델, 호스트 이름, 서비스 프로토콜을 결합해 공유기·PC·휴대폰·TV·프린터·카메라·스피커·Home 유형을 보수적으로 추론한다. 장비가 이름이나 서비스를 공개하지 않으면 정확한 제품 모델을 단정하지 않고 이름 없는 회색 원으로 유지한다. 릴리스 버전은 `1.2.1+6`이다.

### Wave 16 현재 화면 요약 범위 일치

홈 왼쪽 아래 장비 수는 전체 누적 인벤토리가 아니라 그래프에 전달한 동일한 필터 결과의 장비 수를 표시한다. 경고 수 역시 현재 표시 장비에 연결된 warning/critical만 계산하며 정보성 finding은 제외한다. 전체 화면일 때만 장비에 귀속되지 않은 네트워크 경고를 포함한다. 릴리스 버전은 `1.2.2+7`이다.

### Wave 17 공통 상태 배너

검색 진행·완료·중지·오류 등 동적으로 나타나는 모든 상태 배너는 공통 `_MessagePanel`을 사용한다. 일반 배너는 반투명 검정 배경과 흰색 글씨, 오류 배너는 반투명 오류색과 대비되는 글씨를 사용한다. 모든 배너 오른쪽에는 즉시 숨길 수 있는 `X` 버튼을 항상 표시한다. 릴리스 버전은 `1.2.3+8`이다.

### Wave 18 스캔 앱 아이콘과 간결한 배너 문구

앱 아이콘은 중앙 스캔 버튼과 같은 Material `track_changes` 글리프를 사용한다. 어두운 둥근 사각형 바탕과 보라색 그라데이션을 적용하고, 재현 가능한 생성 도구로 Windows 다중 해상도 ICO와 Android mdpi~xxxhdpi 런처 PNG를 함께 만든다.

상태 배너 문구는 `검색 완료 · 신규 장비 1개 발견`, `연결 실패`, `네트워크 준비 중`처럼 짧은 명사형으로 통일한다. 문구는 닫기 버튼과 독립적으로 배너 전체의 중앙에 정렬한다. 릴리스 버전은 `1.3.0+9`이다.
...
| 14 | 장비 유형별 입체 구와 힘 기반 메시 그래프 | 완료 |
| 15 | 단색 다중 GW 메시, 닫을 수 있는 오류 배너와 공개 장비 식별 보강 | 완료 |
| 16 | 현재 화면 장비·경고와 왼쪽 요약 숫자의 범위 일치 | 완료 |
| 17 | 모든 동적 상태 배너의 반투명 배경·가독성·닫기 동작 통일 | 완료 |
| 18 | 중앙 스캔 앱 아이콘과 중앙 정렬 명사형 상태 배너 | 완료 |
...
- [NetBIOS over TCP/UDP RFC 1002](https://www.rfc-editor.org/info/rfc1002/)
- [Microsoft nbtstat](https://learn.microsoft.com/windows-server/administration/windows-commands/nbtstat)
