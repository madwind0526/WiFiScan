...
Bluetooth-Comm의 사용성을 참고해 화면은 아이콘 중심으로 유지한다. 상단에는 검색/설정 아이콘과 Tooltip을 두고, 하단에는 홈·장비·경고 NavigationBar를 둔다. 숫자 지표는 아이콘과 값 위주로 표시하고 상세 설명은 Tooltip, 상세 화면, 스크롤 콘텐츠에서 제공한다. 아이콘만으로 의미가 사라지지 않도록 Tooltip과 Semantics label을 함께 유지한다.
