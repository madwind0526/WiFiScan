...
그래프 위에 동적으로 나타나는 진행·완료·중지·오류 메시지는 하나의 공통 배너 컴포넌트로 처리한다. 일반 상태는 반투명 검정과 흰색 글씨, 오류는 반투명 오류색과 대비 글씨를 사용한다. 본문을 `Expanded`로 감싼 행 끝에 항상 닫기 버튼을 두어 작은 화면과 큰 글꼴에서 긴 메시지가 줄바꿈되어도 버튼이 남게 한다. 닫기는 메시지와 오류 상태를 함께 초기화한다.
