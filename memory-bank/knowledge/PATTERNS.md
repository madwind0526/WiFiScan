...
## Dismissible Overlay Error Banner

그래프 위 오류 배너는 오류색을 반투명으로 합성하고 본문을 `Expanded`로 감싼 행 끝에 닫기 버튼을 둔다. 작은 화면과 큰 글꼴에서 긴 메시지가 줄바꿈되어도 닫기 버튼이 남아야 하며, 닫기는 메시지와 오류 상태를 함께 초기화한다.
